...
// use runfiles::Runfiles;  // Removed: Bazel-specific dependency
use serde_json::json;
use std::fs;
use std::sync::Arc;
...
async fn dashboard_html() -> Html<String> {
    // Try to read the HTML file directly from the static directory
    match fs::read_to_string("static/debug_dashboard.html") {
...
<body>
    ...
    <p>API endpoints are still available at /api/*</p>
</body>
</html>"#
                    .to_string(),
            )
